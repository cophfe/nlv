...
void Spline::GetClosestCurve(glm::vec2 point, int* controlPointIndex, float* curveT)
{
	...
	//now use gradient descent to get a more accurate result (I knew I learned backpropagation for a reason!)
	//although this gradient descent is much easier because the distance function is one dimentional
	//gradient descent will be done on the distance function
	float tValue = localT;
	{
		...
		// GetStepSize function is broke so ill use a constant, this works as long as the point isn't too far away (which it isn't in my use case)
		float stepSize = 0.00001f;

		//iterate and get t
		float gradient = 10000;
		for (size_t i = 0; i < iterations && glm::abs(gradient) > 0.03f; i++)
		{
